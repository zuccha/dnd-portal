...
alter table "public"."spells" add column "duration_value_temp" integer;

UPDATE public.spells
SET duration_value_temp =
  CASE
    WHEN duration_value like '% s' THEN (substring(duration_value from 1 for length(duration_value) - 2)::numeric * 1::integer)::integer
    WHEN duration_value like '% round' THEN (substring(duration_value from 1 for length(duration_value) - 6)::numeric * 6::integer)::integer
    WHEN duration_value like '% min' THEN (substring(duration_value from 1 for length(duration_value) - 4)::numeric * 60::integer)::integer
    WHEN duration_value like '% hr' THEN (substring(duration_value from 1 for length(duration_value) - 3)::numeric * 3600::integer)::integer
    WHEN duration_value like '% d' THEN (substring(duration_value from 1 for length(duration_value) - 2)::numeric * 86400::integer)::integer
    ELSE NULL
  END
WHERE duration_value IS NOT NULL;

alter table "public"."spells" add constraint "spells_duration_pair_chk" CHECK ((((duration = 'value'::public.spell_duration) = (duration_value IS NOT NULL)) AND ((duration = 'value'::public.spell_duration) = (duration_value_temp IS NOT NULL)))) not valid;
