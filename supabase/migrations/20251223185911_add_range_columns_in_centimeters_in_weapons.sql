alter table "public"."weapons" drop constraint "weapons_ranged_range_check";
...
alter table "public"."weapons" add column "range_long" integer;

alter table "public"."weapons" add column "range_short" integer;

UPDATE public.weapons
SET
  range_short = CASE
    WHEN range_ft_short IS NOT NULL THEN (range_ft_short * 30)::integer
    ELSE range_short
  END,
  range_long = CASE
    WHEN range_ft_long IS NOT NULL THEN (range_ft_long * 30)::integer
    ELSE range_long
  END;

alter table "public"."weapons" add constraint "weapons_ranged_range_check" CHECK ((ranged = ((range_short IS NOT NULL) AND (range_long IS NOT NULL) AND (range_ft_short IS NOT NULL) AND (range_ft_long IS NOT NULL) AND (range_m_short IS NOT NULL) AND (range_m_long IS NOT NULL)))) not valid;

alter table "public"."weapons" validate constraint "weapons_ranged_range_check";
...
CREATE OR REPLACE FUNCTION public.fetch_weapons(p_campaign_id uuid, p_langs text[], p_filters jsonb DEFAULT '{}'::jsonb, p_order_by text DEFAULT 'name'::text, p_order_dir text DEFAULT 'asc'::text)
 RETURNS TABLE(id uuid, campaign_id uuid, campaign_name text, type public.weapon_type, damage text, damage_type public.damage_type, damage_versatile text, mastery public.weapon_mastery, properties public.weapon_property[], magic boolean, melee boolean, ranged boolean, range_long integer, range_short integer, range_ft_long real, range_ft_short real, range_m_long real, range_m_short real, weight_kg real, weight_lb real, cost real, name jsonb, notes jsonb, page jsonb, ammunition jsonb, visibility public.campaign_role)
 LANGUAGE sql
 SET search_path TO 'public', 'pg_temp'
AS $function$
WITH prefs AS (
