...
alter table "public"."spells" add column "range_value" integer;

UPDATE public.spells
SET range_value =
  CASE
    WHEN range_value_imp like '% ft' THEN (substring(range_value_imp from 1 for length(range_value_imp) - 3)::numeric * 30::integer)::integer
    WHEN range_value_imp like '% mi' THEN (substring(range_value_imp from 1 for length(range_value_imp) - 3)::numeric * 15000::integer)::integer
    ELSE NULL
  END
WHERE range_value_imp IS NOT NULL;

alter table "public"."spells" add constraint "spells_range_pair_chk" CHECK ((((range = 'value'::public.spell_range) = (range_value_imp IS NOT NULL)) AND ((range = 'value'::public.spell_range) = (range_value_met IS NOT NULL)) AND ((range = 'value'::public.spell_range) = (range_value IS NOT NULL)))) not valid;
