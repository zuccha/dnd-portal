...
BEGIN
  ...
  INSERT INTO public.spells (
    ...
    coalesce(r.character_classes, '{}'::public.character_class[]), r.casting_time, r.casting_time_value,
...
WITH prefs AS (
  SELECT
    ...
    (
      ...
      WHERE e.value = 'true'
    ) AS classes_legacy_inc,
    (
      SELECT coalesce(array_agg(lower(e.key)::public.character_class), null)
      FROM jsonb_each_text(p_filters->'character_classes') AS e(key, value)
      WHERE e.value = 'false'
    ) AS classes_legacy_exc,
...
filtered AS (
  ...
  WHERE
    ...
    AND (p.classes_legacy_inc IS NULL OR s.character_classes && p.classes_legacy_inc)
    AND (p.classes_legacy_exc IS NULL OR NOT (s.character_classes && p.classes_legacy_exc))
