...
alter table "public"."weapons" drop column "range_ft_long";

alter table "public"."weapons" drop column "range_ft_short";

alter table "public"."weapons" drop column "range_m_long";

alter table "public"."weapons" drop column "range_m_short";

alter table "public"."weapons" drop column "weight_kg";

alter table "public"."weapons" drop column "weight_lb";

alter table "public"."weapons" add constraint "weapons_ranged_range_check" CHECK ((ranged = ((range_short IS NOT NULL) AND (range_long IS NOT NULL)))) not valid;
...
CREATE OR REPLACE FUNCTION public.fetch_weapons(p_campaign_id uuid, p_langs text[], p_filters jsonb DEFAULT '{}'::jsonb, p_order_by text DEFAULT 'name'::text, p_order_dir text DEFAULT 'asc'::text)
 RETURNS TABLE(id uuid, campaign_id uuid, campaign_name text, type public.weapon_type, damage text, damage_type public.damage_type, damage_versatile text, mastery public.weapon_mastery, properties public.weapon_property[], magic boolean, melee boolean, ranged boolean, range_long integer, range_short integer, weight integer, cost integer, name jsonb, notes jsonb, page jsonb, ammunition jsonb, visibility public.campaign_role)
 LANGUAGE sql
 SET search_path TO 'public', 'pg_temp'
AS $function$
WITH prefs AS (
