alter table "public"."spells" drop constraint "spells_range_value_imp_check";

alter table "public"."spells" drop constraint "spells_range_value_met_check";

alter table "public"."spells" drop constraint "spells_casting_time_pair_chk";

alter table "public"."spells" drop constraint "spells_duration_pair_chk";

alter table "public"."spells" drop constraint "spells_range_pair_chk";
...
alter table "public"."spells" drop column "casting_time_value_temp";

alter table "public"."spells" drop column "duration_value_temp";

alter table "public"."spells" drop column "range_value_imp";

alter table "public"."spells" drop column "range_value_met";

alter table "public"."spells" add constraint "spells_casting_time_pair_chk" CHECK (((casting_time = 'value'::public.spell_casting_time) = (casting_time_value IS NOT NULL))) not valid;

alter table "public"."spells" validate constraint "spells_casting_time_pair_chk";

alter table "public"."spells" add constraint "spells_duration_pair_chk" CHECK (((duration = 'value'::public.spell_duration) = (duration_value IS NOT NULL))) not valid;

alter table "public"."spells" validate constraint "spells_duration_pair_chk";

alter table "public"."spells" add constraint "spells_range_pair_chk" CHECK (((range = 'value'::public.spell_range) = (range_value IS NOT NULL))) not valid;

alter table "public"."spells" validate constraint "spells_range_pair_chk";
...
CREATE OR REPLACE FUNCTION public.fetch_spells(p_campaign_id uuid, p_langs text[], p_filters jsonb DEFAULT '{}'::jsonb, p_order_by text DEFAULT 'name'::text, p_order_dir text DEFAULT 'asc'::text)
 RETURNS TABLE(id uuid, campaign_id uuid, campaign_name text, level smallint, character_classes public.character_class[], school public.spell_school, casting_time public.spell_casting_time, casting_time_value integer, duration public.spell_duration, duration_value integer, range public.spell_range, range_value integer, concentration boolean, ritual boolean, somatic boolean, verbal boolean, material boolean, name jsonb, description jsonb, materials jsonb, page jsonb, upgrade jsonb, visibility public.campaign_role)
 LANGUAGE sql
 SET search_path TO 'public', 'pg_temp'
AS $function$
WITH prefs AS (
