-- Fix fetch_creatures to always return creatures even when translations are missing
-- Uses the same pattern as spells/weapons: name is always aggregated without filtering

-- Remove function to recreate with fix
DROP FUNCTION "public"."fetch_creatures"(uuid, text[], jsonb, text, text);

-- Update fetch_creatures to ensure creatures are always returned (with unfiltered names)
