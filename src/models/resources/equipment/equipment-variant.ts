...
import { type EquipmentRarity, equipmentRarities } from "~/models/types/equipment-rarity";
...
export function addEquipmentVariant<E extends Equipment>(
  ...
  const id = createDeterministicUuid([base.kind, base.source_id, baseId, modifierIds]);

  return addResource(createEquipmentVariant(base, modifiers, id, { temporary }));
}
...
export function createEquipmentVariant<E extends Equipment>(
  ...
  return {
    ...
    attunement_notes: modifiers.reduce(
      (notes, modifier) => appendI18nString(notes, modifier.attunement_notes_delta, "; "),
      base.attunement_notes,
    ),
    cost: (base.cost ?? 0) + modifiers.reduce((total, modifier) => total + modifier.cost_delta, 0),
    id,
    ...
    rarity: modifiers.reduce(
      (rarity, modifier) => getMaxEquipmentRarity(rarity, modifier.rarity_minimum),
      base.rarity,
    ),
    required_attunement_slots: Math.max(
      base.required_attunement_slots,
      ...modifiers.map(
        ({ required_attunement_slots_minimum }) => required_attunement_slots_minimum,
      ),
    ),
    variant_base_id: base.variant_base_id ?? base.id,
    variant_modifier_ids: [...(base.variant_modifier_ids ?? []), ...modifierIds],
    virtual: temporary,
    weight:
      (base.weight ?? 0) + modifiers.reduce((total, modifier) => total + modifier.weight_delta, 0),
  };
}
...
function getFirstAvailableModifierId(equipment: Equipment): string | undefined {
  return equipment.modifier_ids.find((id) => !(equipment.variant_modifier_ids ?? []).includes(id));
}
...
function getMaxEquipmentRarity(a: EquipmentRarity, b: EquipmentRarity): EquipmentRarity {
