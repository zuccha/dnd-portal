...
export const spellLevelSchema = z.union(spellLevels.map((level) => z.literal(level)));

export const spellLevelStringSchema = z.union(spellLevels.map((level) => z.literal(`${level}`)));
